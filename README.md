...
| Relevance-model comparison and full-corpus relevance screening | **Calendar API-call dates not established by the verified records.** | The six 1,000-row evaluation tables retain measured `elapsed_sec`, but elapsed durations do not identify calendar dates. No request dates are inferred from filenames or file modification times. |
